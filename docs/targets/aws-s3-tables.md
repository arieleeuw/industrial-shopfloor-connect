...
The time interval in milliseconds that triggers writing buffered records to the table, even if the [BufferCount](#buffercount) hasn't been reached. If not specified, records are only written when the [BufferCount](#buffercount) is reached. Deafult is 10000 (10 seconds).
